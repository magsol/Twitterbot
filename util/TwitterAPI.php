...
class TwitterAPI {
  ...
  private function __construct() {
    ...
    $this->oauth = new TwitterOAuth(CONSUMER_KEY, CONSUMER_SECRET, OAUTH_TOKEN, OAUTH_TOKEN_SECRET);
  }
  ...
  public function public_timeline($args = array()) {
    return json_decode($this->doAction('get', 'statuses/public_timeline', $args));
  }
  ...
  public function update($post, $args = array()) {
    return json_decode($this->doAction('post', 'statuses/update', array_merge(array('status' => $post),$args)));
  }
  ...
  public function search($searchstring, $args = array()) {
    $url = 'http://search.twitter.com/search.json?q=' . urlencode($searchstring);
